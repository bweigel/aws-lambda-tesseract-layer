...
ARG TESSERACT_VERSION=5.0.0-alpha-20201231
...
RUN yum -y install gcc gcc-c++ make autoconf aclocal automake libtool \
    ...
    libzip-devel freetype-devel lcms2-devel libwebp-devellib \
    libicu-devel tcl-devel tk-devel pango-devel pangomm-devel cairo-devel; yum clean all
...
RUN mkdir -p ${DIST}/lib && mkdir -p ${DIST}/bin && \
 ...
 #   cp /usr/lib64/libwebp.so.5 ${DIST}/lib/ && \
